                    ...
                    <div class="carousel-inner" role="listbox">
                        ...
                        <div class="carousel-item {{ $index == 0 ? 'active' : '' }} rounded">
                            <img src="{{ asset('storage/' . $product->foto) }}"
                                class="img-fluid w-100 h-100 bg-secondary rounded" alt="Product Slide">
                        </div>
                        @endforeach
                    </div>
    ...
    <div class="container py-5">
        <div class="tab-class text-center">
            <div class="row g-4">
                ...
                <div class="col-lg-8 text-end">
                    <ul class="nav nav-pills d-inline-flex text-center mb-5">
                        <li class="nav-item">
                            <a class="d-flex m-2 py-2 bg-light rounded-pill {{ !request()->get('category') && !request()->get('search') ? 'active' : '' }}"
                                data-bs-toggle="pill" data-bs-target="#tab-0">
                                <span class="text-dark" style="width: 130px;">Semua Produk</span>
                            </a>
                        </li>
                        @foreach ($categories as $category)
                        <li class="nav-item">
                            <a class="d-flex py-2 m-2 bg-light rounded-pill {{ request()->get('category') == $category->id_category || $selectedCategoryId == $category->id_category ? 'active' : '' }}"
                                data-bs-toggle="pill" data-bs-target="#tab-{{ $category->id_category }}">
            ...
            <div class="tab-content">
                <!-- Semua Produk -->
                <div id="tab-0"
                    class="tab-pane fade show {{ !request()->get('category') && !request()->get('search') ? 'active' : '' }}">
                    <div class="row g-4">
                        @foreach ($oneProductPerCategory as $categoryId => $productsInCategory)
                        @foreach ($productsInCategory as $product)
                        <!-- Mengakses produk pertama dalam kategori -->
                            ...
                            <div class="rounded position-relative sweater-item">
                                ...
                                <div class="p-4 border border-secondary border-top-0 rounded-bottom">
                                    <a href="{{ route('detail', $product->id_product) }}"
                                        class="h5">{{ $product->nama_product }}</a>
                                    ...
                                    <div class="d-flex flex-column justify-content-center flex-lg-wrap">
                                        <p class="text-dark fs-5 fw-bold mb-0">Rp.
                                            {{ number_format($product->harga_product, 0, ',', '.') }}
                                        </p>
                                        ...
                                        <form action="{{ route('add.to.cart') }}" method="POST">
                                            ...
                                            <!-- Default quantity 1, bisa ditambah jika ada input quantity -->
                                            <button type="submit"
                ...
                @foreach ($categories as $category)
                <div id="tab-{{ $category->id_category }}"
                    class="tab-pane fade {{ request()->get('category') == $category->id_category || $selectedCategoryId == $category->id_category ? 'show active' : '' }}">
                    <div class="row g-4">
        ...
        <div class="row g-4 align-items-center">
            <div class="col-lg-6">
                <div class="py-4">
                    ...
                    <a href="#" class="banner-btn btn border-2 border-secondary rounded-pill py-3 px-4"
                        style="top: 0; right: 25%;">BUY</a>
                </div>
            </div>
            <div class="col-lg-6">
